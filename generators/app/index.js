...
module.exports = class extends Generator {
  ...
  initializing() {
    process.env.GENERATOR_ENV = process.env.GENERATOR_ENV || 'prod';
    this.sourceRoot(path.join(__dirname, '../templates'));

    if (process.env.GENERATOR_ENV === 'prod') {
      shell.rm('-rf', this.templatePath());
      shell.mkdir(this.templatePath());
    }
    else {
      this.log(chalk.yellow('Running in ' + process.env.GENERATOR_ENV + ' environment.'));
    }
  }
  ...
  async prompting() {
    ...
    if (process.env.GENERATOR_ENV === 'prod') {
      ...
      this.project = { ...this.project, ...require(this.templatePath() + '/' + this.project.name + '-master/yeoman.js') };
      this.log("Project loaded! Let's customize your project.");
    }
    else {
      this.log('Loading project...');
      shell.cd(this.templatePath());
      this.project = { ...this.project, ...require(this.templatePath() + '/yeoman.js') };
      this.log("Project loaded! Let's customize your project.");
    }
